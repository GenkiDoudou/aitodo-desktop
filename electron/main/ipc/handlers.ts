...
export function registerIpcHandlers(getMainWindow: () => BrowserWindow | null): void {
  ...
  ipcMain.handle(IPC.SYNC_REGISTER, (_e, dto: import('@shared/sync-protocol').SyncRegisterRequest) =>
    wrapIpcAsync(() => syncEngine().register(dto))
  )
  ipcMain.handle(
    IPC.SYNC_COMPLETE_LOGIN,
    (_e, request: import('@shared/sync-protocol').SyncCompleteLoginRequest) =>
      wrapIpcAsync(() => syncEngine().completeLogin(request ?? { policy: 'cancel' }))
  )
